...
def test_nao_contribuinte_difal_e_do_remetente_e_entra_na_margem(tabelas):
    ...
    # a carga final do destino é usada como está, e já embute o diferencial
    assert r.icms_pct == pytest.approx(0.1707)
    assert r.difal_pct == pytest.approx(0.1707 - 0.12)


def test_responsabilidade_do_destinatario_custa_menos_para_a_anara(tabelas):
    """O mesmo destino: contribuinte suporta 12%; não contribuinte suporta a carga final."""
    contribuinte = resolver(tabelas, "MG", True, origem_fiscal="NACIONAL",
                            finalidade="USO_CONSUMO")
    nao = resolver(tabelas, "MG", False, origem_fiscal="NACIONAL", finalidade="USO_CONSUMO")
    assert contribuinte.icms_pct < nao.icms_pct


def test_carga_final_nao_e_recalculada(tabelas, session):
    """OK-13 preservado: a coluna Carga Final entra como está, sem base simples/dupla/FEM."""
    ba = session.exec(select(EstadoFiscal).where(EstadoFiscal.uf == "BA")).first()
    r = resolver(tabelas, "BA", False, origem_fiscal="NACIONAL")
