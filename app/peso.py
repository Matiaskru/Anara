...
class PesoResolvido:
    ...
    fonte: Optional[str]
    gsm_efetivo: Optional[Decimal] = None

    def como_dict(self) -> dict:
        return {"peso_kg": para_float(self.peso_kg), "tipo": self.tipo,
                "fonte": self.fonte, "gsm_efetivo": para_float(self.gsm_efetivo)}
...
def resolver_peso(peso_real: Optional[float], largura_cm: Optional[float],
    ...
    if peso_real:
        return PesoResolvido(D(peso_real), "REAL KTC", "Peso informado pela KTC")
    ...
    if largura_cm and comprimento_cm and familia in peso_kg_m2_familia:
        ...
        return PesoResolvido(area * D(peso_kg_m2_familia[familia]), "ESTIMADO",
    ...
    if largura_cm and comprimento_cm:
        ...
        if gsm_efetivo is None and thread_count is not None:
            gsm_efetivo = D(gsm_por_tc.get(str(int(thread_count))))
            origem_gsm = f"tabela fios→GSM ({int(thread_count)} fios)"
        if gsm_efetivo is None and familia:
            gsm_efetivo = D(gsm_por_familia.get(familia))
    ...
    if familia and familia in peso_por_familia:
        return PesoResolvido(D(peso_por_familia[familia]), "TÉCNICO",
