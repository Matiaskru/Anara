...
class ResultadoFrete:
    ...
    cf_logistico: Optional[float] = None       # R$ fixos do embarque
    rv_logistico_pct: Optional[float] = None   # fração da receita
    ...
    componentes_fixos: Dict[str, float] = field(default_factory=dict)
    componentes_percentuais: Dict[str, float] = field(default_factory=dict)
    ...
    def valor_rv(self, receita: float) -> float:
        """RV recomposto em reais — só depois de o preço existir."""
        return (receita or 0.0) * (self.rv_logistico_pct or 0.0)

    def total(self, receita: float) -> float:
        return (self.cf_logistico or 0.0) + self.valor_rv(receita)

    def como_dict(self) -> dict:
        return {
            "status": self.status, "frete_peso": self.frete_peso,
            "cf_logistico": self.cf_logistico, "rv_logistico_pct": self.rv_logistico_pct,
            "peso_real_kg": self.peso_real_kg, "peso_cubado_kg": self.peso_cubado_kg,
            "peso_taxado_kg": self.peso_taxado_kg, "peso_taxado_fonte": self.peso_taxado_fonte,
            "regiao_destino": self.regiao_destino, "tarifa_aplicada": self.tarifa_aplicada,
            "faixa": self.faixa, "minimo_aplicado": self.minimo_aplicado,
            "componentes_fixos": dict(self.componentes_fixos),
            "componentes_percentuais": dict(self.componentes_percentuais),
...
# Peso taxado
# ---------------------------------------------------------------------------
def peso_cubado(volume_m3: Optional[float], fator_kg_m3: Optional[float]) -> Optional[float]:
    if volume_m3 is None or fator_kg_m3 is None:
        return None
    return volume_m3 * fator_kg_m3


def resolver_peso_taxado(peso_real: Optional[float], volume_m3: Optional[float],
                         fator_kg_m3: Optional[float],
                         peso_taxado_confirmado: Optional[float] = None,
...
# Frete-peso e mínimo
# ---------------------------------------------------------------------------
def faixa_para(faixas: Sequence, regiao: str, peso: float):
    ...
    for f in sorted(candidatas, key=lambda f: f.peso_de or 0.0):
        if peso >= (f.peso_de or 0.0) and (f.peso_ate is None or peso <= f.peso_ate):
            return f
    return None


def frete_peso_de(tarifa: float, peso_taxado: float, unidade: str) -> float:
    ...
    if unidade == "BRL_POR_TONELADA":
        return tarifa * peso_taxado / 1000.0
...
def calcular_frete_grupo(tabela, faixas: Sequence, componentes: Sequence,
                         ...
                         peso_real_kg: Optional[float], volume_m3: Optional[float],
    ...
    minimo = faixa.frete_minimo or 0.0
    ...
    base.update(tarifa_aplicada=faixa.tarifa, minimo_aplicado=minimo_aplicado,
                faixa=f"{faixa.peso_de:g}–{faixa.peso_ate:g} {tabela.faixa_unidade}"
                      if faixa.peso_ate else f">{faixa.peso_de:g} {tabela.faixa_unidade}")

    fixos: Dict[str, float] = {"frete_peso": frete_peso}
    ...
    for comp in sorted(vigentes, key=lambda c: c.codigo):
        ...
        if comp.tipo == PERCENTUAL_NF:
            ...
            percentuais[comp.codigo] = float(comp.valor or 0.0)
            ...
            fixos[comp.codigo] = float(comp.valor or 0.0) * peso_do_componente
        elif comp.tipo == FIXO:
            fixos[comp.codigo] = float(comp.valor or 0.0) * float(adicionais_pedidos.get(
                comp.codigo, 1) or 1)
        elif comp.tipo == POR_HORA:
            horas = float(adicionais_pedidos.get(comp.codigo, 0) or 0)
            fixos[comp.codigo] = float(comp.valor or 0.0) * horas
        elif comp.tipo == PERCENTUAL_FRETE:
            fixos[comp.codigo] = float(comp.valor or 0.0) * frete_peso
    ...
    if icms == APLICA and tabela.icms_pct:
        ...
        antes = sum(fixos.values())
        fator = 1.0 - float(tabela.icms_pct)
        if fator <= 0:
            return _bloqueio(REVIEW_REQUIRED, "Alíquota de ICMS do frete inválida.", **base)
        for k in list(fixos):
            fixos[k] = fixos[k] / fator
        avisos.append(f"Gross-up de ICMS de {tabela.icms_pct:.2%} aplicado aos componentes "
                      f"fixos: R$ {antes:,.2f} → R$ {sum(fixos.values()):,.2f}.")
        percentuais = {k: v / fator for k, v in percentuais.items()}

    return ResultadoFrete(
        status=OK, frete_peso=frete_peso, cf_logistico=sum(fixos.values()),
        rv_logistico_pct=sum(percentuais.values()),
...
def _peso_para_componente(tabela, peso_taxado: float, peso_real: Optional[float]):
    ...
    if peso_real is not None and abs((peso_real or 0) - peso_taxado) < 1e-9:
...
# Rateio
# ---------------------------------------------------------------------------
def ratear(valor_total: float, pesos: Sequence[float]) -> List[float]:
    """Rateio proporcional que soma **exatamente** o total.

    A última parcela recebe o resíduo, para que a soma feche ao centavo com o frete do grupo —
    é essa igualdade que torna a cotação reconciliável.
    """
    if not pesos:
        return []
    soma = sum(pesos)
    if soma <= 0:
        # sem base de rateio, divide igualmente — e quem chama registra o critério usado
        parcela = valor_total / len(pesos)
        saida = [parcela] * (len(pesos) - 1)
        return saida + [valor_total - sum(saida)]
    saida = [valor_total * (p / soma) for p in pesos[:-1]]
    return saida + [valor_total - sum(saida)]
